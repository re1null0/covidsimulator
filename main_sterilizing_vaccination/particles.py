...
class Particles():
    ...
    def update_states(self, i, simulator):
        ...
        and stores these results with provided iteration in the dataframe.
    ...
    def get_new_cases_ids(self, i, simulator):
        ...
        # tree = KDTree(contact_ids, leaf_size=2)
        # #global dist, ind_contacts 
        # self.ind_contacts = tree.query_radius(self.x[array, :], r=simulator.init_cont_threshold)
        #all_points = [[contact_ids[idx] for idx in nn_indices] for nn_indices in all_tree_indices]
        # print(contact_ids)
        #print(dist)
        ...
        if len(array) > 1:
            ...
            for k in range(len(contact_ids)):
                if len(contact_ids[k])<=0:
                    continue
                curr_ind = array[k]
                #filtered_app = self.app[(self.app==1)|(self.app==0)]
                if ((self.app[curr_ind]==1) & ((self.epidemic_state[curr_ind]==1) | (self.epidemic_state[curr_ind]==0)| (self.epidemic_state[curr_ind]==2)| (self.epidemic_state[curr_ind]==7))):
                    #contact_ids = np.delete(contact_ids[k], array[k], axis=0)
                    #print(contact_ids)
                    temp_cont = []
                    temp_time = []
                    for j in range (1, len(contact_ids[k])):
                        #m = len(self.contact_cell[curr_ind])
